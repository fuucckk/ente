...
### Why doesn't auth.ente.io recognize my email when I try to log in? {#auth-web-email-not-recognized}
...
2. Clear your browser's site data for `auth.ente.io` (or the cache for that domain).
...
- **Sync check**: log in once on the web app at [auth.ente.io/login](https://auth.ente.io/login) to confirm whether your codes exist in your account at all. If they show up there, the issue is local to one device — try logging out and back in on that device.
